...
__export(src_exports, {
  ...
  isHasDecorators: () => isHasDecorators,
  isHasExpressionInitializer: () => isHasExpressionInitializer,
  isHasInitializer: () => isHasInitializer,
  isHasJSDoc: () => isHasJSDoc,
  isHasModifiers: () => isHasModifiers,
  isHasType: () => isHasType,
  isHasTypeArguments: () => isHasTypeArguments,
...
function forEachToken(node, callback, sourceFile = node.getSourceFile()) {
  ...
  const queue = [];
  while (true) {
    if (isTS4dot3 && ts2.isTokenKind(node.kind)) {
      callback(node);
...
function canHaveTrailingTrivia(token) {
  switch (token.kind) {
    case ts3.SyntaxKind.CloseBraceToken:
      return token.parent.kind !== ts3.SyntaxKind.JsxExpression || !isJsxElementOrFragment(token.parent.parent);
    case ts3.SyntaxKind.GreaterThanToken:
      switch (token.parent.kind) {
        case ts3.SyntaxKind.JsxOpeningElement:
          return token.end !== token.parent.end;
        case ts3.SyntaxKind.JsxOpeningFragment:
          return false;
        ...
        case ts3.SyntaxKind.JsxClosingFragment:
...
function isJsxElementOrFragment(node) {
  return node.kind === ts3.SyntaxKind.JsxElement || node.kind === ts3.SyntaxKind.JsxFragment;
}
function forEachComment(node, callback, sourceFile = node.getSourceFile()) {
  const fullText = sourceFile.text;
  const notJsx = sourceFile.languageVariant !== ts3.LanguageVariant.JSX;
  return forEachToken(
    node,
    (token) => {
      if (token.pos === token.end)
        return;
      if (token.kind !== ts3.SyntaxKind.JsxText)
        ts3.forEachLeadingCommentRange(
          fullText,
          token.pos === 0 ? (ts3.getShebang(fullText) ?? "").length : token.pos,
          commentCallback
        );
      if (notJsx || canHaveTrailingTrivia(token))
        return ts3.forEachTrailingCommentRange(
          fullText,
          token.end,
          commentCallback
        );
...
function isCompilerOptionEnabled(options, option) {
  switch (option) {
    ...
    case "allowSyntheticDefaultImports":
      return options.allowSyntheticDefaultImports !== void 0 ? options.allowSyntheticDefaultImports : isCompilerOptionEnabled(options, "esModuleInterop") || options.module === ts4.ModuleKind.System;
...
function isModifierFlagSet(node, flag) {
  return isFlagSet(ts5.getCombinedModifierFlags(node), flag);
}
...
function isNumberKeyword(node) {
  return node.kind === ts6.SyntaxKind.NumberKeyword;
}
function isObjectKeyword(node) {
  return node.kind === ts6.SyntaxKind.ObjectKeyword;
}
function isOutKeyword(node) {
  return node.kind === ts6.SyntaxKind.OutKeyword;
}
function isOverrideKeyword(node) {
  return node.kind === ts6.SyntaxKind.OverrideKeyword;
}
function isPrivateKeyword(node) {
  return node.kind === ts6.SyntaxKind.PrivateKeyword;
}
function isProtectedKeyword(node) {
  return node.kind === ts6.SyntaxKind.ProtectedKeyword;
}
...
function isReadonlyKeyword(node) {
  return node.kind === ts6.SyntaxKind.ReadonlyKeyword;
}
function isStaticKeyword(node) {
  return node.kind === ts6.SyntaxKind.StaticKeyword;
}
function isStringKeyword(node) {
  return node.kind === ts6.SyntaxKind.StringKeyword;
}
function isSuperExpression(node) {
  return node.kind === ts6.SyntaxKind.SuperKeyword;
}
function isSuperKeyword(node) {
  return node.kind === ts6.SyntaxKind.SuperKeyword;
}
function isSymbolKeyword(node) {
  return node.kind === ts6.SyntaxKind.SymbolKeyword;
}
function isSyntaxList(node) {
  return node.kind === ts6.SyntaxKind.SyntaxList;
}
function isThisExpression(node) {
  return node.kind === ts6.SyntaxKind.ThisKeyword;
}
function isThisKeyword(node) {
  return node.kind === ts6.SyntaxKind.ThisKeyword;
}
function isTrueKeyword(node) {
  return node.kind === ts6.SyntaxKind.TrueKeyword;
}
function isTrueLiteral(node) {
  return node.kind === ts6.SyntaxKind.TrueKeyword;
}
function isUndefinedKeyword(node) {
  return node.kind === ts6.SyntaxKind.UndefinedKeyword;
}
function isUnknownKeyword(node) {
  return node.kind === ts6.SyntaxKind.UnknownKeyword;
}
function isUnparsedPrologue(node) {
  return node.kind === ts6.SyntaxKind.UnparsedPrologue;
}
function isUnparsedSyntheticReference(node) {
  return node.kind === ts6.SyntaxKind.UnparsedSyntheticReference;
}
function isVoidKeyword(node) {
  return node.kind === ts6.SyntaxKind.VoidKeyword;
}
...
function isDeclarationWithTypeParameterChildren(node) {
  return isSignatureDeclaration(node) || isClassLikeDeclaration(node) || ts7.isInterfaceDeclaration(node) || ts7.isTypeAliasDeclaration(node) || ts7.isJSDocTemplateTag(node);
}
...
function isHasExpressionInitializer(node) {
...
function isHasTypeArguments(node) {
...
function isObjectTypeDeclaration(node) {
  return isClassLikeDeclaration(node) || ts7.isInterfaceDeclaration(node) || ts7.isTypeLiteralNode(node);
}
...
function isTypeOnlyCompatibleAliasDeclaration(node) {
  return ts7.isImportClause(node) || ts7.isImportEqualsDeclaration(node) || ts7.isNamespaceImport(node) || ts7.isImportOrExportSpecifier(node);
}
...
function isJsxTagNamePropertyAccess(node) {
  return ts8.isPropertyAccessExpression(node) && isJsxTagNameExpression(node.expression);
}
...
var ts14 = __toESM(require("typescript"), 1);

// src/types/typeGuards/compound.ts
var ts13 = __toESM(require("typescript"), 1);

// src/types/typeGuards/objects.ts
var ts12 = __toESM(require("typescript"), 1);

// src/types/typeGuards/simple.ts
var import_typescript = __toESM(require("typescript"), 1);
function isBigIntLiteralType(type) {
  return isTypeFlagSet(type, import_typescript.default.TypeFlags.BigIntLiteral);
}
function isConditionalType(type) {
  return isTypeFlagSet(type, import_typescript.default.TypeFlags.Conditional);
}
function isEnumType(type) {
  return isTypeFlagSet(type, import_typescript.default.TypeFlags.Enum);
}
function isIndexType(type) {
  return isTypeFlagSet(type, import_typescript.default.TypeFlags.Index);
}
function isIndexedAccessType(type) {
  return isTypeFlagSet(type, import_typescript.default.TypeFlags.IndexedAccess);
}
function isInstantiableType(type) {
  return isTypeFlagSet(type, import_typescript.default.TypeFlags.Instantiable);
}
function isIntersectionType(type) {
  return isTypeFlagSet(type, import_typescript.default.TypeFlags.Intersection);
}
function isNumberLiteralType(type) {
  return isTypeFlagSet(type, import_typescript.default.TypeFlags.NumberLiteral);
}
function isObjectType(type) {
  return isTypeFlagSet(type, import_typescript.default.TypeFlags.Object);
}
function isStringLiteralType(type) {
  return isTypeFlagSet(type, import_typescript.default.TypeFlags.StringLiteral);
}
function isStringMappingType(type) {
  return isTypeFlagSet(type, import_typescript.default.TypeFlags.StringMapping);
}
function isSubstitutionType(type) {
  return isTypeFlagSet(type, import_typescript.default.TypeFlags.Substitution);
}
function isTemplateLiteralType(type) {
  return isTypeFlagSet(type, import_typescript.default.TypeFlags.TemplateLiteral);
}
function isTypeParameter(type) {
  return isTypeFlagSet(type, import_typescript.default.TypeFlags.TypeParameter);
}
function isTypeVariable(type) {
  return isTypeFlagSet(type, import_typescript.default.TypeFlags.TypeVariable);
}
function isUnionType(type) {
  return isTypeFlagSet(type, import_typescript.default.TypeFlags.Union);
}
function isUnionOrIntersectionType(type) {
  return isTypeFlagSet(type, import_typescript.default.TypeFlags.UnionOrIntersection);
}
function isUniqueESSymbolType(type) {
  return isTypeFlagSet(type, import_typescript.default.TypeFlags.UniqueESSymbol);
}

// src/types/typeGuards/objects.ts
function isEvolvingArrayType(type) {
  return isObjectType(type) && isObjectFlagSet(type, ts12.ObjectFlags.EvolvingArray);
}
function isTupleType(type) {
  return isObjectType(type) && isObjectFlagSet(type, ts12.ObjectFlags.Tuple);
}
function isTypeReference(type) {
  return isObjectType(type) && isObjectFlagSet(type, ts12.ObjectFlags.Reference);
}
...
var ts16 = __toESM(require("typescript"), 1);

// src/nodes/utilities.ts
var ts15 = __toESM(require("typescript"), 1);
